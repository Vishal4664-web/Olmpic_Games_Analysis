...
def most_succesful(df,sport):
    temp_df=df.dropna(subset=['Medal']) # all the rows dropped whuch has NaN value
    if sport != 'overall':
...
def most_succesful_country_wise(df,country):
    temp_df=df.dropna(subset=['Medal']) # all the rows dropped whuch has NaN value
...
def weight_v_height(df,sport):
    ...
    if sport!='overall': # not selected by teh selected bar
