...
st.sidebar.image('lympiclogo.png')
...
if user_menu =='Country Wise Analysis':
    ...
    st.sidebar.title("Country Wise Analysis") # Sidebar is used for show in menu-bar  
...
if user_menu == 'Athlete Wise Analysis':
    ...
    famous_sport=['Basketball','Judo','Football','Tug of war','Athletics','Swimming','Badminton',
              ...
              'synchronized Swimming','table Tennis','Baseball','Rhythmic Gymnastic',
    ...
    fig.update_layout(autosize=False,width=1000,height=600)
    st.title("Distribution of Age wiith respect to sports(Only Gold Medalist)")
    st.plotly_chart(fig) 
    ...
    st.pyplot(fig) #Just an image snapshot of the chart.

    st.title("Men vs Women participation over the year")
    final=helper.men_vs_women(df)
    fig=px.line(final,x='Year',y=['Male','Female'])
    fig.update_layout(autosize=False,width=1000,height=600)
    st.plotly_chart(fig) # Retains full interactivity (zoom, hover, pan).